...
chatServices.findByIdAndUpdate = (chatId,value,filter) => {
    switch(filter){
        case "messagesId":
            return Chat.findByIdAndUpdate(chatId, { $set: { messagesId: value || [] } }, {useFindAndModify:false},(err) => {
                return err;
            });
        case "position":
            return Chat.findByIdAndUpdate(chatId, { $set: { position: value || 0 } },{useFindAndModify:false}, (err) => {
                return err;
            });
        default: 
            return Chat.findByIdAndUpdate(chatId, { $set: { messagesId: value || [] } },{useFindAndModify:false}, (err) => {
